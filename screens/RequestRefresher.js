...
const RequestRefresher = ({navigation}) => {
  ...
  const handleRequestData = () => {
    const dataToSend = {
      ...
      submissionType:
        submissionType === 0
          ? 'Submit to Scheduling'
          : 'Do Not Submit to Scheduling',
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <Card style={styles.card}>
          ...
          <SelectList
            data={timeOptions.map(item => ({key: item, value: item}))}
          ...
          {/* Refresher Details */}
          <SelectList
            data={refresherTypes.map(item => ({key: item, value: item}))}
            setSelected={setSelectedRefresherType}
            placeholder="Refresher Type"
            boxStyles={styles.selectListBox}
          />

          <SelectList
            data={chargeCodes.map(item => ({key: item, value: item}))}
            setSelected={setSelectedChargeCode}
            placeholder="Charge Code"
            boxStyles={styles.selectListBox}
          />

          <SelectList
            data={refresherReasons.map(item => ({key: item, value: item}))}
            setSelected={setSelectedRefresherReason}
            placeholder="Refresher Reason"
            boxStyles={styles.selectListBox}
          />

          <Input
            label="Authorization Comments"
            value={authComments}
            onChangeText={setAuthComments}
            style={styles.input}
          />
          <Input
            label="Authorize PIN"
