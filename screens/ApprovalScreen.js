...
import { StyleSheet, SafeAreaView, View, Platform, Alert, TextInput} from "react-native";
import {
  Layout,
  Text,
  Button,
...
const Approve = ({ navigation }) => {
  ...
  const HandleAuthorization = async (authtype) => {
    ...
    const response = await fetch(
      `${
        authUser.host
      }content?module=home&page=m&reactnative=1&mode=authrequest&reqtype=${
        pdata.reqtype
      }&requestid=${
        pdata.requestid
      }&hrsappr=${hours}&authtype=${authtype}&pinnum=${pin}&comment=${comment}&submittoscheduling=${submitToScheduling}&persid=${
        authUser.currpersid
      }&uname=${authUser.uname}&password=${authUser.upwd}&customer=eta${
        authUser.schema
      }&session_id=${authUser.sessionid}&etamobilepro=1&nocache=${
    ...
    const PendData = {
      pdataid: String(pdata.requestid), // Ensure ID is a string
    };
    navigation.popTo("PendingAuth",{PendData})
  };
  ...
  const handleConfirm = () => {
    if (etaresponse.status == "-1") {
      ...
      //      Alert.alert("Alert", etaresponse.msg);
      //      navigation.goBack();
      ...
      Alert.alert(
        ...
        [
          ...
          //{text: 'OK', onPress: console.log(123456)},
          {
            text: "OK",
      ...
      <SafeAreaView>
        <View style={styles.content}>
          ...
          <Text category="p2" appearance="hint" style={styles.subtitle}>
            {pdata.duration > 0 ? pdata.duration : pdata.hour} Hours
          </Text>
